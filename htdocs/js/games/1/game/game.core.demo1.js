	...
	var _game = {
		...
		player: {
			...
			create: function() {
				...
				// Create a HingeConstraint to limit player's air-twisting - this needs improvement AAAAAAAAAAAAAAAAAAAAAAAAAAAAA
			...
			updateOrientation: function() {
				...
				if ((_cannon.getCollisions(_game.player.rigidBody.index) &&
					((_game.player.rotationAngleX >= 90) ||
						(_game.player.rotationAngleX <= -0) ||
						(_game.player.rotationAngleY >= 0) ||
						(_game.player.rotationAngleY <= -0)))
					)
				{
					// Reset orientation
		...
		level: {
			...
			create: function() {
				...
				_cannon.createRigidBody({
					...
					meshMaterial: new THREE.MeshLambertMaterial({color: 0x32a852}),
				...
				// Add some boxes
					...
					meshMaterial: new THREE.MeshLambertMaterial({ color: window.game.static.colors.soccer }),
					physicsMaterial: _cannon.solidMaterial });
